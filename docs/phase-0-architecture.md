...
1. 앱을 실행하고 `트래킹 시작`을 누른다. 그 전에는 화면 공유 권한 요청이 나타나지 않아야 한다.
2. 알림 권한 또는 화면 공유를 취소한다. UI가 `대기 중`으로 돌아가야 한다.
3. 화면 공유를 승인한다. UI가 `트래킹 중`이 되고 지속 알림이 보여야 한다.
4. 앱의 `트래킹 중지`, 알림의 `트래킹 중지`, Android 상태바의 화면 공유 중지 중 하나를 실행한다. 모두 `대기 중`으로 돌아가야 한다.
5. 기기 화면을 잠근다. Android가 화면 공유를 종료한 뒤 앱이 `대기 중`으로 복구되어야 한다.

## #15 수동 검증

1. 트래킹 중에만 `AFK 분석` 버튼이 보여야 한다.
2. 버튼 1회 → 현재 프레임 1장이 디버그 미리보기에 표시되어야 한다.
3. 캡처 직후 `ImageReader`는 분리·해제되어야 한다.
4. 회전·캡처 영역 크기 변경 후에도 다음 캡처가 표시되어야 한다.
5. 트래킹 종료 시 미리보기와 캡처 리소스가 정리되어야 한다.

이 Phase는 Bitmap 미리보기만 제공한다. OCR·화면 판별·저장·자동 분석은 구현하지 않는다.
